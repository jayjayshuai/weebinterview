const AreaJson = [{
    ...
    "city": [{
        ...
        "name": "焦作市",
        "area": ["解放区", "中站区", "马村区", "山阳区", "沁阳市", "孟州市", "修武县", "温县", "武陟县", "博爱县","济源市",  "其他"]
    }, {
        "name": "濮阳市",
...
export function ProvinceList(all) {
    var provinces = [];
...
export function CityList(provinceIndex) {

    var citys = [];
    for (var i = 0; i < AreaJson[provinceIndex].city.length; i++) {
        // console.log(AreaJson[provinceIndex].city[i]);
        citys.push(AreaJson[provinceIndex].city[i].name);
    }
    return citys;
}
export function AreaList(provinceIndex, cityIndex) {
    var areas = [];
    areas = AreaJson[provinceIndex].city[cityIndex].area;
    return areas;
}

export function AreaByCity(province, city) {
    var areas = [];
    AreaJson.some(provinceItem=>{
        if(provinceItem.name == province){
            provinceItem.city.some(cityItem=>{
